...
public class MachineManagerImpl implements MachineManager, StopWorkspaceHandler {
    ...
    public void onDevMachineCreating(MachineConfigDto machineConfig) {
        ...
        if (machineConfig.getLink(Constants.LINK_REL_GET_MACHINE_LOGS_CHANNEL) != null &&
            ...
            for (LinkParameter linkParameter : machineConfig.getLink(Constants.LINK_REL_GET_MACHINE_LOGS_CHANNEL).getParameters()) {
                if ("channel".equals(linkParameter.getName())) {
                    outputChannel = linkParameter.getDefaultValue();
                }
            }
            for (LinkParameter linkParameter : machineConfig.getLink(Constants.LINK_REL_GET_MACHINE_STATUS_CHANNEL).getParameters()) {
                if ("channel".equals(linkParameter.getName())) {
